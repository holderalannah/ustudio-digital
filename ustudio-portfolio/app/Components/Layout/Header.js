      ...
      <div className="flex mx-auto w-[90%] max-w-6xl justify-between items-center relative lg:justify-between xl:max-w-7xl">
        ...
        /> */}
         <Nav
          ...
          navigate={() => setOpen(false)}
          linkClasses={`${isHome && !scrolledPastHero && 'after:bg-white!'} ${isContactUs && 'text-white after:bg-white!'} ${scrolledPastHero && isHome ? 'after:bg-darkPink transition delay-150 duration-300 ease-in-out' : ''}`}
        />
      </div>
    </header>
  );
